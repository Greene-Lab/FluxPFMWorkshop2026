...
def create_raw_data(subid, data_dir, task, fd_thresh, TR, num_of_sessions):
        ...
        for visit in visits_to_process:
            ...
            fd_mat_path = glob.glob(os.path.join(visit,'Motion',f'*task-{task}_power_2014_FD_only.mat'))[0]
            # logger.debug(fd_mat_path)
            fd_mat = scipy.io.loadmat(fd_mat_path)
            struct_ind = len((np.arange(0, fd_thresh, .01)))
            fd_label = fd_mat['motion_data'][0,struct_ind]['FD_threshold'][0,0][0][0]
            censor_vect = fd_mat['motion_data'][0,struct_ind]['frame_removal'][0,0]
            if fd_label == fd_thresh:
                pass
            else:
                sys.exit('Something went wrong getting the motion censor index from the matlab file. Check. Exiting...')
            delete_list = np.concatenate(censor_vect,axis=0).tolist()
            del_vector = np.asarray(delete_list)
            delete_indices = list(np.where(del_vector == True)[0])
...
def plot_final(all_data_dict, outdir, subid, truetime, fd_thresh, rands, num_of_sessions, subids):
        ...
        # save out file
        plt.savefig(os.path.join(outdir,f'{subid}_reliability_true{truetime}min_{fd_thresh}FD_5contig_{num_of_sessions}visits_{rands}_Iterations.png'), dpi=1200, format='png', bbox_inches='tight')
